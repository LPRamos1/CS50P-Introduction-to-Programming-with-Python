# Module 01: Conditionals

This module focuses on implementing decision-making logic in Python. These exercises demonstrate how to handle different execution paths based on user input and numerical data.

## 🚀 Key Learning Points
* **Control Flow:** Using `if`, `elif`, and `else` to manage program logic.
* **Input Normalization:** Ensuring robust comparisons by cleaning strings with `.lower()` and `.strip()`.
* **Data Parsing:** Splitting strings into usable components for calculations.
* **Numerical Ranges:** Validating if a value falls within specific time or numeric intervals.
